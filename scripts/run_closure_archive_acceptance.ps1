...
try {
    ...
    $hasClosurePanel = $html.Contains("closure-archive-panel")
    Add-Check "closure_archive_panel_visible" $hasClosurePanel "panel_visible=$hasClosurePanel"

    $appJsResp = Invoke-WebRequest -UseBasicParsing -Uri "$BaseUrl/static/app.js" -TimeoutSec 12
    $appJsText = [string]$appJsResp.Content
    $hasStageDef = $appJsText.Contains("closure_archive")
    Add-Check "closure_archive_stage_defined" $hasStageDef "stage_defined_in_app_js=$hasStageDef"
...
$file = Join-Path $OutputDir ("closure_archive_acceptance_{0}.json" -f $now.ToString("yyyyMMdd_HHmmss"))
$report | ConvertTo-Json -Depth 8 | Set-Content -Path $file -Encoding UTF8

Write-Host "Closure archive stage acceptance report:"
