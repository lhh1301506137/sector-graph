param(
    ...
    [object]$RefreshBeforeCheck = $true
)
...
$checks = @()
$snapshot = [ordered]@{}
$refreshEnabled = To-Bool -Value $RefreshBeforeCheck -Default $true
...
try {
    ...
    $hasPanel = $html.Contains("maintenance-mode-panel")
    Add-Check "maintenance_panel_visible" $hasPanel "panel_visible=$hasPanel"

    $appJsResp = Invoke-WebRequest -UseBasicParsing -Uri "$BaseUrl/static/app.js" -TimeoutSec 12
    $appJsText = [string]$appJsResp.Content
    $hasStageDef = $appJsText.Contains("maintenance_mode")
    Add-Check "maintenance_stage_defined" $hasStageDef "stage_defined_in_app_js=$hasStageDef"
...
$report = [ordered]@{
    ...
    base_url = $BaseUrl
    refresh_before_check = $refreshEnabled
    passed = $allPassed
    checks = $checks
    snapshot = $snapshot
}

$file = Join-Path $OutputDir ("maintenance_mode_acceptance_{0}.json" -f $now.ToString("yyyyMMdd_HHmmss"))
$report | ConvertTo-Json -Depth 8 | Set-Content -Path $file -Encoding UTF8

Write-Host "Maintenance mode acceptance report:"
