...
try {
    ...
    $hasObservePanel = $html.Contains("post-release-observe-panel")
    $hasArchivePanel = $html.Contains("post-release-observe-archive")
    Add-Check "post_release_panel_visible" $hasObservePanel "panel_visible=$hasObservePanel"
    Add-Check "post_release_archive_visible" $hasArchivePanel "archive_visible=$hasArchivePanel"

    $appJsResp = Invoke-WebRequest -UseBasicParsing -Uri "$BaseUrl/static/app.js" -TimeoutSec 12
    $appJsText = [string]$appJsResp.Content
    $hasStageDef = $appJsText.Contains("post_release_observe")
    Add-Check "post_release_stage_defined" $hasStageDef "stage_defined_in_app_js=$hasStageDef"
    ...
    Add-Check "sync_heartbeat_ok" $syncOk "last_sync_at=$lastSyncAt, compare_status=$compareStatus"
...
$file = Join-Path $OutputDir ("post_release_observe_acceptance_{0}.json" -f $now.ToString("yyyyMMdd_HHmmss"))
$report | ConvertTo-Json -Depth 8 | Set-Content -Path $file -Encoding UTF8

Write-Host "Post release observe acceptance report:"
